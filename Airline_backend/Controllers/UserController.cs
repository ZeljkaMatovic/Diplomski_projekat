...
namespace BackEnd.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        //{
        //    string userId = User.Claims.First(c => c.Type == "UserID").Value;
        //    var user = _dataBase.Users.Include(u => u.ReservedCars).ThenInclude(r => r.Vehicle).ToList().Find(u => u.UserName == userId);
        ...
        //{
        //    string userId = User.Claims.First(c => c.Type == "UserID").Value;
        //    var user = _dataBase.Users.Include(u => u.ReservedCars).ToList().Find(u => u.UserName == userId);
        ...
        //    var res =_dataBase.CarReservations.Include(res => res.Vehicle).ToList().Find( c => c.Id == resId);
        //    var date = res.StartDate;
        //    date.AddDays(2);
        //    if (date <= DateTime.Now)
        //        return BadRequest("Deadline for canceling reservation has passed");

        //    var car = _dataBase.Vehicles.Include(v => v.DatesTaken).ToList().Find(v => v.Id == res.Vehicle.Id);
        ...
        //    var vehicle = _dataBase.Vehicles.Include(v => v.DatesTaken).Include(v => v.SpecialOfferDates).ToList().Find(v => v.Id == info.VehicleID);
        ...
        //{
        //    string userId = User.Claims.First(c => c.Type == "UserID").Value;
        //    var user = _dataBase.Users.ToList().Find(u => u.UserName == userId);

        //    if (user == null || user.Role != "ru")
        //        return Unauthorized();

        //    var rent = _dataBase.Vehicles.Include(v => v.DatesTaken).Include(v => v.SpecialOfferDates).ToList().FindAll(v => v.RentacarID == info.RentacarID);
        ...
        public async Task<Object> FindRoundAwayTrips()
        {
            ...
            await Task.Run(() =>
            {
                ...
                user = _dataBase.Users.Include(u => u.ReservedTickets).ToList().Find(u => u.UserName == userId);
            });
